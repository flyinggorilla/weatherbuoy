...
unsigned short WebClient::HttpExecute() {
	...
	// Socket
	int clientSocket = socket(res->ai_family, res->ai_socktype, 0);
	if (clientSocket < 0) {
	...
	if (write(clientSocket, sRequest.c_str(), sRequest.length()) < 0) {
	...
	if (mpPostData) {
		if (write(clientSocket, mpPostData, muPostDataSize) < 0) {
	...
	while (!mHttpResponseParser.ResponseFinished()) {
		size_t sizeRead = read(clientSocket, (char*)sReceiveBuf.c_str(), sReceiveBuf.length());
